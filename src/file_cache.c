...
static uint32_t hash_file_name(uint32_t h, const char* const* file_name) {
    return hash_uint64(h, (uintptr_t)file_name);
}

static bool file_name_is_equal(const char* const* file_name, const char* const* other_file_name) {
    return *file_name == *other_file_name;
}

MAP_DEFINE(cached_file_map, const char*, struct cached_file*, hash_file_name, file_name_is_equal, PRIVATE)
...
static struct cached_file* alloc_cached_file(const char* file_data, size_t file_size) {
...
struct file_cache {
    struct cached_file_map cached_files;
...
struct file_cache* file_cache_create(void) {
    ...
    file_cache->cached_files = cached_file_map_create();
...
const struct cached_file* file_cache_read(struct file_cache* file_cache, const char* file_name) {
    const char* canonical_file_name = canonicalize_file_name(file_cache, file_name);

    struct cached_file* const* existing_file = cached_file_map_find(&file_cache->cached_files, &canonical_file_name);
    if (existing_file)
        return *existing_file;

    size_t file_size = 0;
    char* file_data = read_file(canonical_file_name, &file_size);
    struct cached_file* cached_file = file_data ? alloc_cached_file(file_data, file_size) : NULL;
    free(file_data);

    [[maybe_unused]] bool was_inserted = cached_file_map_insert(&file_cache->cached_files, &canonical_file_name, &cached_file);
    assert(was_inserted);
    return cached_file;
}
